...
import net.engyne.dao.RuleTemplateLogDao;
 				    		...
 				    		for(int i=0;i<rules.size();i++)
 	 				    	{
	 				    		...
	 				    		if(users.size()>0)
	 				    		{
	 				    			...
	 				    			for(int j=0;j<templates.size();j++)
	 				    			{
	 				    				...
	 				    				RuleDao.updateRuleCount(rules.get(i).getRule_index());
	 				    				TemplateDao.updateTemplateCount(templates.get(j).getTemplate_index());
	 				    				RuleTemplateLogDao.updateRuleTemplateLog(rules.get(i).getRule_index(), templates.get(j).getTemplate_index(), clientid, System.currentTimeMillis()/1000+"");
	 				    			}
